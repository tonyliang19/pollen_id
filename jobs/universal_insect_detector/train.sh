...
PROGRAM=universal_insect_detector.py
...
case "$1" in
       ...
       train)
            ...
            ;;
       predict)
            sbatch --job-name=$0$1   --account=${SLURM_ACCOUNT}   --gres=gpu:p100:1 --cpus-per-task=2 --mem=16000M --time=0-02:00 --output=$output --wrap="${command} --gpu"
           ;;
        *)
            echo $"Wrong action"
            exit 1
