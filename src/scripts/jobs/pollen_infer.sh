...
case "$2" in
       train)
            ...
            ;;
       predict)
            sbatch --job-name="${JOBNAME}"   --account="${SLURM_ACCOUNT}" --gres=gpu:p100:1 --cpus-per-task=2 --mem=16000M --time=1-00:00 --mail-user=chunqingliang@gmail.com --mail-type=ALL --output="outs/$output" --wrap="${command}"
           ;;
      validate)
